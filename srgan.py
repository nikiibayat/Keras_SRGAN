...
def fid_score(images1, images2):
    ...
    model=InceptionV3(include_top=False, input_shape=(75, 75, 3), pooling='avg')
...
if __name__ == '__main__':
    ...
    gan.generator.load_weights('saved_model/ProcessedCelebA16to64.h5')
    rootdir = "/home/nbayat5/celebA/identities"
    for subdir, dirs, files in os.walk(rootdir):
        for dir in dirs:
            ...
            if len(parts) == 5:
                continue
            # print("path", path)
            imgs_hr, imgs_lr = gan.data_loader.load_dataforIdentities(path)
            fake_hr = gan.generator.predict(imgs_lr)
            images = 0.5 * fake_hr + 0.5
            fake_hr = np.asarray(fake_hr)
            path2 = "./SRGAN_%s.png" % (parts[5].rstrip())
            print("path save img: ", path2)
            imageio.imwrite(path2, np.squeeze(fake_hr))
            break


    ##########PSNR - SSIM - FID ###############
    # with tf.Session() as sess:
    #     print("CelebA Tensorflow PSNR: ", np.mean(tf.image.psnr(imgs_hr, fake_hr, max_val=1).eval()))
    #     print("CelebA Tensorflow SSIM: ", np.mean(tf.image.ssim(tf.image.convert_image_dtype(imgs_hr, tf.float32),tf.image.convert_image_dtype(fake_hr, tf.float32),max_val=1).eval()))
